...
export function setRequestLocals(locals: any) {
  // 不再使用模組級變數儲存狀態
  console.log('API: setRequestLocals (deprecated) - 建議直接傳遞 locale 參數')
}
...
function getCurrentLocale(): Locale {
  ...
  if (typeof window === 'undefined') {
    console.log('API: SSR 環境，建議明確傳遞 locale 參數，使用預設值:', defaultLocale)
  ...
  if (langParam && (langParam === 'zh-tw' || langParam === 'en')) {
    ...
    console.log('API: 使用 URL 參數語系:', locale)
    return locale
  }
  ...
  if (match && (match[1] === 'zh-tw' || match[1] === 'en')) {
    ...
    console.log('API: 使用 cookie 語系:', locale)
    return locale
  }
  ...
  if (acceptLanguage) {
    ...
    console.log('API: 使用瀏覽器語言設定:', locale)
    return locale
  }

  console.log('API: 使用預設語系:', locale)
  return locale
}
...
>(
  ...
  if (options?.locale) {
    ...
    console.log("API: 使用明確傳遞的語系:", frontendLocale)
  } else {
    // 否則使用自動檢測
  ...
  console.log("API Language - Frontend:", frontendLocale, "Backend:", backendLocale)
